...
func (s *baseServer) maybeSendInvalidToken(p *receivedPacket, hdr *wire.Header) error {
	...
	extHdr, err := unpackHeader(opener, hdr, p.data, hdr.Version)
	if err != nil {
		// don't return the error here. Just drop the packet.
		return nil
	}
	hdrLen := extHdr.ParsedLen()
	if _, err := opener.Open(p.data[hdrLen:hdrLen], p.data[hdrLen:], extHdr.PacketNumber, p.data[:hdrLen]); err != nil {
		// don't return the error here. Just drop the packet.
		return nil
	}
	if s.logger.Debug() {
